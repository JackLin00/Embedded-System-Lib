...
参数说明:
ts:时间戳信息,4字节
timeZone:时区,相对格林威治时间的偏移
buffer:用于存储转换结果的缓冲区,转换后的结果每位占4字节---顺序为年，月，日，时，分，秒
*/
void praseTimestamp(uint32_t ts,int16_t timeZone,uint32_t *buffer) {
    uint8_t dayOfMonth[] = {31,28,31,30,31,30,31,31,30,31,30,31};       //每月的日数
    //闰年标记
    uint8_t index = 0;
    uint8_t leapYear = 0;
    uint8_t i = 0;
    ...
    //中国区需要补28800秒
    //计算时间戳的天数
    uint32_t days = (ts + 3600 * timeZone) / 86400 + 1;
    //四年周期数
    int16_t year4 = (int16_t)(days / 1461);
    //不足四年周期剩下的天数
    int16_t remain = (int16_t)(days % 1461);
    ...
    buffer[ss] = (int16_t)(ts % 60);

    buffer[mm] = (int16_t)(ts /60 % 60);
    ...
    if(ts == 86400){
        ...
        //中国区需要+8h
        buffer[HH] =(int16_t)(ts / 3600) + timeZone;
    }
